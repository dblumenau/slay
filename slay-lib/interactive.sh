...
# Discover URL for a project using the configured discovery script
# Returns URL if found, empty string if not
discover_url() {
    local group="$1"
...
⚡ demo"
            ...
            enter_url)
                ...
                # Try to discover URL via plugin, then check cache
                local discovered_url cached_url
                discovered_url=$(discover_url "$selected_customer")
                cached_url=$(get_cached_url "$selected_customer")

                # Build options based on what we found
                ...
                if [ "$has_options" = true ]; then
                    ...
                    local choices=()
                    [ -n "$discovered_url" ] && choices+=("Use discovered URL")
